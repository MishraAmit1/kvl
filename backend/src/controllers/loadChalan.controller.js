...
const generateChalanNumber = async () => {
  ...
  // Get count of chalans for today
  const todayStart = new Date(date.getFullYear(), date.getMonth(), date.getDate());
  const todayEnd = new Date(date.getFullYear(), date.getMonth(), date.getDate() + 1);
  
  const count = await LoadChalan.countDocuments({
...
const createLoadChalan = asyncHandler(async (req, res) => {
  ...
  if (!bookingBranch || !destinationHub || !consignments || consignments.length === 0) {
  ...
  const existingConsignments = await Consignment.find({ _id: { $in: consignmentIds } });
  ...
  const chalanNumber = await generateChalanNumber();

  // Create consignments array with snapshot data
  const consignmentsWithSnapshot = consignments.map(consignment => {
    const existing = existingConsignments.find(c => c._id.toString() === consignment.consignmentId);
    return {
...
const getAllLoadChalans = asyncHandler(async (req, res) => {
  const { page = 1, limit = 10, status, date, vehicleNumber, driverName } = req.query;
  ...
  if (vehicleNumber) query["vehicle.vehicleNumber"] = { $regex: vehicleNumber, $options: 'i' };
  if (driverName) query["driver.driverName"] = { $regex: driverName, $options: 'i' };
...
const getLoadChalanById = asyncHandler(async (req, res) => {
  ...
  return sendResponse(res, 200, loadChalan, "Load Chalan retrieved successfully");
});
...
const updateLoadChalanStatus = asyncHandler(async (req, res) => {
  ...
  const validStatuses = ["CREATED", "DISPATCHED", "IN_TRANSIT", "ARRIVED", "CLOSED"];
  ...
  const statusOrder = ["CREATED", "DISPATCHED", "IN_TRANSIT", "ARRIVED", "CLOSED"];
  const currentIndex = statusOrder.indexOf(currentStatus);
  const newIndex = statusOrder.indexOf(status);

  if (newIndex < currentIndex) {
    throw throwApiError(400, `Cannot move back from ${currentStatus} to ${status}`);
  }
  ...
  return sendResponse(res, 200, loadChalan, "Load Chalan status updated successfully");
});
...
const generateLoadChalanPDFController = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=load-chalan-${loadChalan.chalanNumber}.pdf`);
    res.send(pdfBuffer);
  } catch (error) {
