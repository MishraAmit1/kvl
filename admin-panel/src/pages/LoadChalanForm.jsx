...
const LoadChalanForm = () => {
  ...
  const mutation = useMutation({
    mutationFn: (data) => 
      isEditing ? loadChalanApi.updateLoadChalan(id, data) : loadChalanApi.createLoadChalan(data),
    onSuccess: () => {
      toast.success(isEditing ? 'Load Chalan updated successfully' : 'Load Chalan created successfully');
  ...
  useEffect(() => {
    ...
    const uniqueConsignments = selectedConsignments.filter((consignment, index, self) => 
      index === self.findIndex(c => c.consignmentId === consignment.consignmentId)
    );
  ...
  const handleConsignmentSelect = (consignment) => {
    ...
    const isAlreadySelected = selectedConsignments.some(c => c.consignmentId === consignment._id);
    
    // Also check by consignment number as backup
    const isAlreadySelectedByNumber = selectedConsignments.some(c => c.consignmentNumber === consignment.consignmentNumber);
    
    if (isAlreadySelected || isAlreadySelectedByNumber) {
      toast.error(`Consignment ${consignment.consignmentNumber} is already selected`);
      return;
    }
  ...
  const allConsignments = consignmentsData?.data?.consignments || consignmentsData?.data || [];
  ...
  const selectedConsignmentIds = selectedConsignments.map(c => c.consignmentId);
  ...
  const consignments = allConsignments.filter(c => {
    ...
    const isAlreadySelectedByNumber = selectedConsignments.some(selected => selected.consignmentNumber === c.consignmentNumber);
    
    const isAlreadySelected = isAlreadySelectedById || isAlreadySelectedByNumber;
    ...
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => handleInputChange('bookingBranch', e.target.value)}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Destination Hub</label>
              <input
                type="text"
                value={formData.destinationHub}
                onChange={(e) => handleInputChange('destinationHub', e.target.value)}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (vehicle) {
                    handleInputChange('vehicle.vehicleNumber', vehicle.vehicleNumber);
                    handleInputChange('vehicle.engineNumber', vehicle.engineNumber);
                    handleInputChange('vehicle.chassisNumber', vehicle.chassisNumber);
                  }
                }}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleInputChange('vehicle.vehicleNumber', e.target.value)}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                readOnly
              />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (driver) {
                    ...
                    handleInputChange('driver.driverLicenseNo', driver.licenseNumber);
                  }
                }}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleInputChange('driver.driverName', e.target.value)}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                readOnly
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => handleInputChange('driver.driverMobile', e.target.value)}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                readOnly
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Cleaner Name</label>
              <input
                type="text"
                value={formData.driver.cleanerName}
                onChange={(e) => handleInputChange('driver.cleanerName', e.target.value)}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">Available Consignments</label>
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-gray-600">
                        {consignment.fromCity} → {consignment.toCity} | {consignment.packages} pkgs | {consignment.actualWeight} kg
                      </div>
                    </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Selected Consignments</label>
                ...
                {selectedConsignments.map((consignment, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-md">
                    <div>
                      <div className="font-medium">{consignment.consignmentNumber}</div>
                      <div className="text-sm text-gray-600">
                        {consignment.destination} | {consignment.packages} pkgs | {consignment.weight} kg
                      </div>
                    </div>
                    <button
                      type="button"
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Lorry Freight</label>
              <input
                type="number"
                value={formData.lorryFreight}
                onChange={(e) => handleInputChange('lorryFreight', parseFloat(e.target.value) || 0)}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleInputChange('tdsDeduction', parseFloat(e.target.value) || 0)}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Freight Payable At</label>
              <input
                type="text"
                value={formData.frtPayableAt}
                onChange={(e) => handleInputChange('frtPayableAt', e.target.value)}
                className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Risk Note</label>
              <select
